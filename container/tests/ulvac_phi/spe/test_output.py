...
class TestOutputCase1:
    ...
    inputdata: Union[str, List[str]] = "XPS_1block.spe"

    def test_setup(self):
        setup_inputdata_folder(self.inputdata, "ulvac_phi", "spe")

    def test_raw_data(self, setup_main, data_path):
        assert os.path.exists(os.path.join(data_path, "nonshared_raw", "XPS_1block.spe"))

    def test_main_image(self, data_path):
        assert os.path.exists(os.path.join(data_path, "main_image", "XPS_1block.png"))

    def test_structured(self, data_path):
        assert os.path.exists(os.path.join(data_path, "structured", "XPS_1block_Al2p.csv"))
        assert os.path.exists(os.path.join(data_path, "structured", "XPS_1block.txt"))

    def test_thumbnail(self, data_path):
        assert os.path.exists(os.path.join(data_path, "thumbnail", "XPS_1block.png"))
